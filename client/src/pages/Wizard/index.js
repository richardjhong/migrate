...
// import { useCountryData } from "../../utils/adjustCountryData";
// import CountryRecs from "../../components/CountryRecs";
import './Wizard.scss'
...
function CountryRecs() {
    ...
    topFive = revisedCountryRanks.slice(0,5)
    console.log(topFive)
    // setButtonClicked(false)
    

    return (
    <div className='topFiveResults'>
            <h2>Highest Score Countries:</h2>
            {topFive.map((country)=> (
                <div className='revCountryResult'>
                <button className='topCountryButton'>{country.countryName}</button>
                <h3>revised SPI = {country.revisedSPI}</h3>
                <h3>original SPI = {country.origSPI}</h3>
                </div>
            ))}
        </div>
    )
